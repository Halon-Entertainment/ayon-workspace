...
@click.command(
    name="addons",
    help="Creates packages and uploads them to the server configured in the .env file.",
)
...
def upload_addons_cli(
    addons, all_addons=False, create_package_only=False, restart_server=False
):
    if not all_addons:
        vaild_addons = set(
            [x.name for x in ADDONS_FOLDER.iterdir() if x.is_dir()]
        )
