...
def auto_version_pyproject():
    ...
    if "version" in pyproject["tool"]["poetry"]:
        ...
        if not semantic_version.validate(version):
            raise RuntimeError(f"Invalid version in pyproject.toml: '{version}'")
    else:
    ...
    with open("pyproject.toml", "w") as stream:
        toml.dump(pyproject, stream)

    return str(version)

def apply_version_to_package_py(version):
    with open("package.py", "r") as stream:
        content = stream.read()

    content = re.sub(r'version = "(.*?)"', f'version = "{version}"', content)

    with open("package.py", "w") as stream:
        stream.write(content)

    return version

def upload_and_restart_server(package_zip):
    ayon_api.upload_addon_zip(package_zip)
...
def main(
    ...
    auto_version: Optional[bool] = False,
    ...
    if auto_version:
        version = auto_version_pyproject()
        apply_version_to_package_py(version)
    ...
    if auto_upload:
        upload_and_restart_server(os.path.join(output_dir, f"{ADDON_NAME}-{ADDON_VERSION}.zip"))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--auto-version",
        action="store_true",
        help="Automatically update version in client code. This is useful for development."
    )
    parser.add_argument(
            "--auto-upload",
            action="store_true",
            help="Automatically upload the package to the server."
    )
    ...
    print(args.auto_version, args.auto_upload)
    main(args.output_dir, args.skip_zip, args.keep_sources, args.only_client, args.auto_version, args.auto_upload)
